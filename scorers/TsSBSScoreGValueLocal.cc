...
#include "TsSBSScoreGValueLocal.hh"
...
TsSBSScoreGValueLocal::TsSBSScoreGValueLocal(TsParameterManager* pM, TsMaterialManager* mM, TsGeometryManager* gM, TsScoringManager* scM, TsExtensionManager* eM,
...
TsSBSScoreGValueLocal::~TsSBSScoreGValueLocal()
{
	;
}
...
G4bool TsSBSScoreGValueLocal::ProcessHits(G4Step* aStep, G4TouchableHistory*)
{
	if (!fIsActive) {
	...
	if (-1 < aTrack->GetTrackID()) {
		...
		if (0 < fNbOfMoleculesToScavenge) {
			...
			G4double t = aTrack->GetLocalTime();
...
void TsSBSScoreGValueLocal::UserHookForEndOfEvent()
{
...
void TsSBSScoreGValueLocal::AbsorbResultsFromWorkerScorer(TsVScorer* workerScorer)
{
	TsVNtupleScorer::AbsorbResultsFromWorkerScorer(workerScorer);

	TsSBSScoreGValueLocal* workerGvalueScorer = dynamic_cast<TsSBSScoreGValueLocal*>(workerScorer);
...
void TsSBSScoreGValueLocal::Output()
{
...
void TsSBSScoreGValueLocal::Clear()
{
